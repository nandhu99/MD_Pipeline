...
Heads (-alhead):
  ...
  head bead types supported:
    ...
    I1 = C1 = Inositol part 1   - bead P1, charge  0
    I2 = C2 = Inositol part 2   - bead P4, charge  0
    I3 = C3 = Inositol part 3   - bead P4, charge  0
  ...
  Examples of lipid heads:
    ...
    "I1 I2 I3 P" -> 'C1 C2 C3 PO4' - PI - PhosphatidylInositol
  ...
  PI head group included in this script based on Lopez 2013 JCTC by Nandhitha Subramanian on Nov 8th, 2016.
...
options = [
    """
    Options:""",
    ("-o", Option(str, 1, "Martini-lipid.itp", "Output specific Martini lipid topology")),
    ...
    ("-desc",
     Option(str, 1, "This is a ...", "A general description of what the FF is / represents, only use in comments")),
...
# PO4 = Phosphate (one bond x2 charges can be used e.g. when making unprotonated PA lipids)
}
...
progString = "The Martini lipid itp generator version " + __version__ + "  Args are: -o %s -alname %s -alhead '%s' -allink '%s' -altail '%s'" % (
itpFileName, lipidName, lipidHead, lipidLinker, lipidTail)
print progString
...
if len(headsArray) > 0 and headsArray[0] == 'PI':  # Add PI head
    ...
    anglesArray.append(
        [-1, '4    5    6         2 120.00 25.0 ; These are part of the default lipids rules but not used here'])
    anglesArray.append(
        [-1, '4    5    7         2 180.00 25.0 ; These are part of the default lipids rules but not used here'])
    anglesArray.append([-1, 'Tail part (uses standar Martini v2.0 tail rules)'])

    dihedralsArray.append(
        [-1, '3  1  4  5  1  -30.0  5.0  1 ; Removed as it was unstable - WARNING has not been tested'])
    ...
    anglesArray.append(
        [-1, '4    6    7         2 120.00 25.0 ; These are part of the default lipids rules but not used here'])
    anglesArray.append(
        [-1, '4    6    8         2 180.00 25.0 ; These are part of the default lipids rules but not used here'])
    anglesArray.append([-1, 'Tail part (uses standar Martini v2.0 tail rules)'])

    dihedralsArray.append(
        [-1, '3  1  4  6  1  -30.0  5.0  1 ; Removed as it was unstable - WARNING has not been tested'])
    ...
    dihedralsArray.append(
        [-1, '3  1  4  7  1  -30.0  5.0  1 ; Removed as it was unstable - WARNING has not been tested'])
    ...
    bondsArray.append([-2, '#endif'])
    bondsArray.append([5, 6, '0.60', '25000', '; Always keep as bond for stability'])
    bondsArray.append([6, 7, '0.60', '25000',
                       '; New to add P3 bead - not messured just placed, added just as bonds to increase stability'])
    bondsArray.append([1, 4, '0.35', defBforce, ''])
    ...
    anglesArray.append(
        [-1, '4    8    9         2 120.00 25.0 ; These are part of the default lipids rules but not used here'])
    anglesArray.append(
        [-1, '4    8   10         2 180.00 25.0 ; These are part of the default lipids rules but not used here'])
    anglesArray.append([-1, 'Tail part (uses standar Martini v2.0 tail rules)'])

    dihedralsArray.append(
        [-1, '3  1  4  8  1  -30.0  5.0  1 ; Removed as it was unstable - WARNING has not been tested'])
    ...
    constraintsArray.append(
        [-1, '6  7  1  0.60 ; New to add P3 bad - not messured just placed, added just as bonds to increase stability'])
    constraintsArray.append([-2, '#endif'])
    ...
    if len(linkersArray) > 1:
        if len(tailsArray[0]) == 1 and tailsArray[0][
            0] == '-':  # First tail is missing (like in LPC) keep <head - linker_1 - linker_2> straight
...
for linkerBeadIndex in range(0, len(linkersArray)):
    ...
    if (cLinker == "G"):  # GLY
        if len(tailsArray[linkerBeadIndex]) == 1 and tailsArray[linkerBeadIndex][
            0] == '-':  # No tail given for current linker bead so free OH group Na changed to P1
            beadArray.append([index, 'P1', 1, lipidName, "GL" + str(linkerBeadIndex + 1), index, 0,
                              ''])  # Example of this is in LPC (it as not tail A so GL1 changes to P1 type
...
for cTail in tailsArray:
    ...
    for cTailBeadIndex in range(0, len(cTail)):
        ...
        if cTailBead == 'C':
            ...
            if (cTailBeadIndex > 0 and cTail[cTailBeadIndex - 1] == 'D') or (
                    len(cTail) > (cTailBeadIndex + 1) and cTail[cTailBeadIndex + 1] == 'D'):
...
for cBead in beadArray:
    if cBead[0] > 0:
        print >> itpFile, '  %2i \t%s \t%2i \t%s \t%s \t%2i \t%s \t%s' % (
        cBead[0], cBead[1], cBead[2], cBead[3], cBead[4], cBead[5], cBead[6], cBead[7])
...
for cAngle in anglesArray:
    if cAngle[0] > 0:
        print >> itpFile, '  %2i %2i %2i \t2 \t%s \t%s \t%s' % (
        cAngle[0], cAngle[1], cAngle[2], cAngle[3], cAngle[4], cAngle[5])
...
if len(dihedralsArray) > 0:
    ...
    for cDihedral in dihedralsArray:
        if cDihedral[0] > 0:
            print >> itpFile, '  %2i %2i %2i %2i \t%i \t%s \t%s \t%s' % (
            cDihedral[0], cDihedral[1], cDihedral[2], cDihedral[3], cDihedral[4], cDihedral[5], cDihedral[6],
            cDihedral[7])
...
if len(constraintsArray) > 0:
    ...
    for cConstraint in constraintsArray:
        if cConstraint[0] > 0:
            print >> itpFile, '  %2i %2i \t1 \t%s \t%s' % (
            cConstraint[0], cConstraint[1], cConstraint[2], cConstraint[3])
